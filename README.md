...
В данной работе представлен пет проект по анализу данных. Генерируются две выборки со смещенным средним и проводятся A/B и A/A тесты. Язык - Python
...
1. Задаем N (количество пользователей) для контрольной и тестовой групп. В данном случае N = 5000.
...
3. Генерируем user CTR для двух группы, где среднее для контрольной группы равно success_rate, а для тестовой группы -
   success_rate * (1 + uplift). Пусть uplift = 0.2.
5. Генерируем clicks для обеих групп.
...
В основном нулевая и альтернативная гипотезы звучат так: H_0 - средние контрольной и тестовой групп равны, а H_1 - средние не равны. Уровень значимости примем равным 0.05.
Для каждого A/B-теста мы проведём ряд статистических тестов и сравним их эффективность, построив ROC-кривую и расчитав мощность.
...
Также для проверки корректности работы тестов будем проводить A/A-тесты. Используем те же шаги генерации данных, но с uplift = 0. Значит распределения тестовой и контрольной групп будут одинаковы, следовательно, нулевая гипотеза будет истинна. Для A/A-тестов распределение p value равномерно на отрезке [0, 1], а ROC-кривая имеет диагональный вид. Для оценки консервативности и антиконсервативности теста будем рассчитывать доверительный интервал Уилсона.
...
2. t - тест
   ...
   Используем функцию stats.ttest_ind, которая будет сравнивать математические ожидания количеств нажатий на рекламу для групп A и B (clicks).
...
3. Пуассоновский бутстрэп
   ...
   Бутстрэп считается трудоёмким с вычислительной точки зрения при работе с большими данными, но для расчёта глобального CTR можно применить пуассоновский бутстрап (Poisson bootstrap).
   Вместо повторной выборки пользователей генерируем для каждого пользователя число его появлений в выборке из распределения Пуассона. Затем взвешиваем просмотры и клики на это число.
...
```
   ...
   Простая и эффективная альтернатива бутстрэпу — бакетизация. Вместо того чтобы извлекать независимые выборки, разбиваем всех пользователей на небольшое количество корзин (bucket) и
   рассчитываем глобальный CTR для каждой корзины. Каждую группу можно рассматривать как «мета-пользователя», а глобальный CTR корзины — как CTR этого мета-пользователя. Затем можно применит t-тест к этим мета-пользователям, поскольку корзины независимы.
...
Как можно заметить, на конкретно этих данных t-тест был менее эффективен. Тест Манна-Уитни довольно простой, быстрый и имеет хорошую мощность. Более навороченные тесты бутстрэп и бакетизация показали самую большую мощность.
Но перед применением тестов надо учитывать некоторые допущения:

1. Эксперимент не влияет на views.

2. CTR пользователей и uplift не зависят от views.

Если не выполняется ни первое, ни второе, следует рассматривать только clicks (t-тест или тест Манна-Уитни). В наших симуляциях последний в целом показал большую мощность, но требует гораздо больше вычислительных ресурсов, чем t-тест.

Если первое допущение выполняется, а второе — нет, можно использовать методы для глобального CTR. В данных симуляциях бутстрэп оказался наилучшим выбором. Если на каких-то данных он вычислительно неосуществим, следует рассмотреть альтернативы, например, бакетизацию.
